...
resource method for handling POST requests.  Write a REST client to test the
service that performs both GET and POST requests using different values
...
the output by using the parameter ~zone~.  If ~zone~ is not provided the clock
...
Finally, compare your solution with the solution of [[file:~/Documents/CorsoFirenze/exercises/ex1/exercises1.org][Exercise 2 of problem set 1]].
